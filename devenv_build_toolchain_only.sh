...
docker exec -i lucet-toolchain sh -c 'cat > /opt/lucet/bin/wasm32-unknown-wasi-clang; chmod 755 /opt/lucet/bin/wasm32-unknown-wasi-clang' <<EOT
#! /bin/sh
exec "clang" -fno-trapping-math -mthread-model single -W,--no-threads --target="wasm32-unknown-wasi" --sysroot="/opt/wasi-sysroot" "\$@"
EOT
